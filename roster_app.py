...
with tab_row_tracker:
  ...
  st.markdown(
      "Track completed rows independently per task using the slider or quick"
      " buttons, add notes for missing/skipped rows, and view estimated"
      " remaining work."
  )
  ...
  for task_name in row_progress_tasks:
    ...
    if isinstance(task_data, (int, float)):
      task_data = {"completed": int(task_data), "notes": ""}
      st.session_state.task_row_progress[task_name] = task_data

    curr_completed = task_data.get("completed", 0)
    curr_notes = task_data.get("notes", "")

    slider_key = f"slider_{task_name}"
    if slider_key not in st.session_state:
      st.session_state[slider_key] = curr_completed
    ...
    if c_b1.button("➕ 10 Rows", key=f"p10_{task_name}"):
      new_val = min(
          260,
          st.session_state.task_row_progress[task_name].get("completed", 0)
          + 10,
      )
      st.session_state.task_row_progress[task_name]["completed"] = new_val
    ...
    if c_b3.button("Reset", key=f"preset_{task_name}"):
      st.session_state.task_row_progress[task_name]["completed"] = 0
    ...
    if c_b4.button("Complete All", key=f"pall_{task_name}"):
      st.session_state.task_row_progress[task_name]["completed"] = 260
    ...
    def update_task_slider(t=task_name):
      ...
      if not isinstance(curr_sets["task_row_progress"].get(t), dict):
        curr_sets["task_row_progress"][t] = {"completed": 0, "notes": ""}
      val = st.session_state[f"slider_{t}"]
      curr_sets["task_row_progress"][t]["completed"] = val
      save_settings(curr_sets)
    ...
    st.session_state.task_row_progress[task_name]["completed"] = slider_val
    ...
    if new_notes != curr_notes:
      st.session_state.task_row_progress[task_name]["notes"] = new_notes
      save_settings({
          "completed_rows_count": saved_settings.get(
              "completed_rows_count", 0
          ),
    ...
    st.progress(min(1.0, slider_val / 260.0))
